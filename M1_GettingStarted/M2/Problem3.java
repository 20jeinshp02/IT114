...
public class Problem3 {
    ...
    static <T> void bePositive(T[] arr){
        ...
        if (Integer.class.isInstance(arr[i])) {
        output[i] = Math.abs((Integer) arr[i]);
        }
        else if (Double.class.isInstance(arr[i])) {
        output[i] = Math.abs((Double) arr[i]);
        }
        else if (String.class.isInstance(arr[i])) {
        output[i] = Math.abs(Integer.valueOf((String) arr[i]));
        }
        }
        //end edit section
